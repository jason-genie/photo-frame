...
var cw = (defaultWidth * ratioCanWin) > 250 ? (defaultWidth * ratioCanWin) : 250;
...
window.updatePreview = function(url) {
  ...
  fabric.Image.fromURL(url, function(img) {
    ...
    var oImg = img.set({ left: 0, top: 0}).scale(resizeRatio);
    canvas.add(oImg);
  });
...
window.freshCanvas = function(){
  fabric.Image.fromURL($(".design.active").attr("src"), function(img) {
    ...
    ocw = canvas.width;
    cw = (winWidth * ratioCanWin) > 250 ? (winWidth * ratioCanWin) : 250;
    canvas.setWidth(cw);
